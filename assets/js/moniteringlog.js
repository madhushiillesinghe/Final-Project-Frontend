...
function searchLogs() {
  ...
  logRows.forEach((row) => {
    const logCode = row
      ...
      .textContent.toLowerCase(); // Get the text of the first td
    const logDate = row
      .querySelector("td:nth-child(2)")
      .textContent.toLowerCase(); // Get the text of the second td

    if (logCode.includes(searchTerm) || logDate.includes(searchTerm)) {
